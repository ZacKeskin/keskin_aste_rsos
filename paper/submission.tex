...
\section{Results with Real Data} \label{s.results}
  ...
  \begin{figure*}[!htbp]
    \includegraphics[width=\linewidth]{images/BTC_6_quantile_bins.pdf}
    ...
    \includegraphics[width=\linewidth]{images/LTC_6_quantile_bins.pdf} 
  ...
  \begin{figure*}[!htbp]
    \includegraphics[width=\linewidth]{images/XRP_6_quantile_bins.pdf}
    ...
    \includegraphics[width=\linewidth]{images/ETH_6_quantile_bins.pdf}
...
This quantity depends on the relative volumes of the bins, and hence do the estimates for $H(Y)$ and $H(X,Y)$ also require these to be calculated. However, in the mutual information $I(X;Y) = H(X)+H(Y)-H(X,Y)$ the terms containing the volumes cancel, and its expression turns out to be dependent only on the relative frequencies:
 ...
 \! \!\sum_{x_k,y_j} \!\!\frac{n(x_k,y_j)}{N } \log\left({  \frac{n(x_k,y_j)}{N} }\right)
  ...
  Table \ref{t.table3} contains results quantifying the net information transfer across windows of the study for each cryptocurrency. Net information transfer is calculated as $ \operatorname{TE}^{(1)}_{X \rightarrow Y} - \operatorname{TE}^{(1)}_{Y \rightarrow X} $, and is given in units of bits.
